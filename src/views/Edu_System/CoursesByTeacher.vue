                    ...
                    <el-main>
                        ...
                        <el-form :inline="true" :model="searchForm" class="demo-form-inline">
                            ...
                            <el-form-item label="添加课程">
                                <el-button type="primary" @click="goToCoursesByTeacher_Add">点击进入课程列表</el-button>
                            </el-form-item>
                        </el-form>
                        <!-- 表格 -->
                        <el-table :data="paginatedCoursesData" border ref="courseTable">
                            ...
                            <el-table-column align="center" prop="ctime" label="上课时间" width="150">
                                <template slot-scope="scope">
                                    <span>{{ formatClassTime(scope.row.ctime) }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column align="center" prop="cloc" label="上课地点" width="125">
                            </el-table-column>
                            ...
                            <el-table-column align="center" label="操作">
                                <template slot-scope="scope">
                                    <el-button type="primary"
                                        @click="goToCoursesByTeacher_Class(scope.row.cno, scope.row.cname)">点击进入班级详情</el-button>
                                    <el-button @click="confirmDelete(scope.row.cno)">删除</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                        <!-- 弹窗表格 -->
                        <el-dialog title="课程查询结果" :visible.sync="showTable">
                            ...
                            <el-table :data="courseTable" border width="75%">
                                <el-table-column align="center" prop="ctime" label="上课时间" width="175">
                                    <template slot-scope="scope">
                                        <span>{{ formatClassTime(scope.row.ctime) }}</span>
                                    </template>
                                </el-table-column>
                                <el-table-column align="center" prop="cloc" label="上课地点" width="150">
                                </el-table-column>
                        ...
                        <el-pagination background layout="sizes, prev, pager, next, jumper, total"
                            ...
                            :current-page="currentPage" :total="totalRows" :page-size="pageSize" :page-sizes="[1, 2, 3, 4]">
...
export default {
    ...
    methods: {
        ...
        async searchCourseBycno() {
            if (this.searchForm.cno) {
                axios.get(`/CoursesOffered?sno=${this.userData.uno}&cno=${this.searchForm.cno}`).then(result => {
        ...
        async searchCourseBycname() {
            if (this.searchForm.cname) {
                axios.get(`/CoursesOffered?sno=${this.userData.uno}&cname=${this.searchForm.cname}`).then(result => {
                    this.courseTable = result.data.data;
                    this.openDialogTable();
                });
            }
        ...
        async deleteCourse(TOcno) {
            axios.delete(`/CoursesOffered?tno=${this.userData.uno}&cno=${TOcno}`).then(result => {
        ...
        confirmDelete(cno) {
            ...
            this.$confirm('确认删除？')
                .then(() => {
                    this.deleteCourse(TOcno); // 执行删除操作
                })
                .catch(() => {
                    // 用户取消删除，什么也不做
                });
        ...
        refresh() { //刷新
            axios.get("/CoursesOffered?uno=${this.userData.uno}").then((result) => {
                this.coursesOffered = result.data.data;
        ...
        batchDelete() {
            ...
            axios.delete(`/CoursesOffered?cnos=${selectedCourses.join(',')}`)
                .then(result => {
                    console.log(result)
                    this.refresh()
        ...
        goToCoursesByTeacher_Class(TOcno, TOcname) {
            const usercno = {
                "cno": TOcno,
                "cname": TOcname
            }
            localStorage.setItem('usercno', JSON.stringify(usercno));
            this.$router.push('/CoursesByTeacher_Class',);
        },
        goToCoursesByTeacher_Add() {
            this.$router.push('/CoursesByTeacher_Add');
        },
    ...
    computed: {
        totalRows() {
            return this.coursesOffered.length; // 总数据条数
        },
        paginatedCoursesData() {
            // 根据当前页数和每页显示的数量，计算截取的数据段
            const startIndex = (this.currentPage - 1) * this.pageSize;
            const endIndex = startIndex + this.pageSize;
            return this.coursesOffered.slice(startIndex, endIndex);
        },
    },
    mounted() {
        this.userData = this.$route.meta.userData;
        axios.get(`/CoursesOffered?uno=${this.userData.uno}`).then((result) => { // "https://mock.apifox.com/m1/3839966-0-default/CoursesOffered"
            this.coursesOffered = result.data.data;
            this.$message.success('数据已更新');
        }); //发送异步请求
    },
