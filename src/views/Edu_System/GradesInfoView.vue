        ...
        <el-container>
          <el-main>
            ...
            <el-form :inline="true" :model="searchForm" class="demo-form-inline">
              ...
              <el-form-item>
                <el-button type="primary" @click="searchGradeBycno">查询</el-button>
              </el-form-item>
              <el-form-item label="课程名称">
                <el-input v-model="searchForm.cname" length="4" show-word-limit placeholder="请输入课程名称"></el-input>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" @click="searchGradeBycname">查询</el-button>
              </el-form-item>
            </el-form>
            ...
            <!-- 表格 -->
            <el-table :data="paginatedstudentGrades" border>
...
export default {
  ...
  methods: {
    ...
    async searchGradeBycno() {
      if (this.searchForm.cno) {
        axios.get(`/StudentGrades?sno=${this.userData.uno}&cno=${this.searchForm.cno}`).then(result => {
    ...
    async searchGradeBycname() {
      if (this.searchForm.cname) {
        axios.get(`/StudentGrades?sno=${this.userData.uno}&cname=${this.searchForm.cname}`).then(result => { // "https://mock.apifox.com/m1/3839966-0-default/StudentGrades?apifoxApiId=139355975"
          this.gradeTable = result.data.data;
          this.$message.success('查询成功');
          this.openDialogTable();
        });
  ...
  computed: {
    totalRows() {
      return this.studentGrades.length; // 总数据条数
    },
    paginatedstudentGrades() {
      // 根据当前页数和每页显示的数量，计算截取的数据段
      const startIndex = (this.currentPage - 1) * this.pageSize;
      const endIndex = startIndex + this.pageSize;
      return this.studentGrades.slice(startIndex, endIndex);
    },
  },
  mounted() {
    this.userData = this.$route.meta.userData;
    axios.get(`/AllGrades?uno=${this.userData.uno}`)
      .then(response => {
