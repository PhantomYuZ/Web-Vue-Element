    ...
    <div>
        <el-container style="height: 650px; border: 1px solid #eee">
            ...
            <el-container>
                ...
                <el-container>
                    <el-main>
                        <el-button type="primary" icon="el-icon-back" @click="goBack_CoursesByTeacher">返回</el-button>
                        <!-- 弹窗表格 -->
                        <el-dialog title="课程查询结果" :visible.sync="showTable">
                            ...
                            <br>
                            <el-table-item>
                                <el-button type="primary" @click="postCourse(courseTable.cno)">添加课程</el-button>
                            </el-table-item>
                        </el-dialog>
                        <hr>
                        <h4>可添加课程</h4>
                        ...
                        <el-form :inline="true" :model="searchForm" class="demo-form-inline">
                            <el-form-item label="课程号">
                                <el-input v-model="searchForm.cno" length="4" show-word-limit
                                    placeholder="按课程号添加教授课程"></el-input>
                            </el-form-item>
                            <el-form-item>
                            ...
                            <el-form-item label="课程名">
                                <el-input v-model="searchForm.cname" length="4" show-word-limit
                                    placeholder="按课程名称添加教授课程"></el-input>
                            </el-form-item>
                            <el-form-item>
                        ...
                        <!-- 表格 -->
                        <el-table :data="paginatedCoursesData" border ref="coursesTable">
                            ...
                            <el-table-column align="center" prop="ctime" label="上课时间" width="300">
                                <template slot-scope="scope">
                                    <span>{{ formatClassTime(scope.row.ctime) }}</span>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column align="center" label="操作">
                                <template slot-scope="scope">
                                    <el-button slot="reference" @click="postCourse(scope.row.cno)">添加</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                        <br>
                        <el-button @click="batchPost()">批量添加</el-button>
...
export default {
    ...
    methods: {
        ...
        async searchCourseBycno() {
            if (this.searchForm.cno) {
                axios.get(`/UntaughtCourses?cno=${this.searchForm.cno}`).then(result => {
        ...
        async searchCourseBycname() {
            if (this.searchForm.cname) {
                axios.get(`/UntaughtCourses?cname=${this.searchForm.cname}`).then(result => {
                    this.courseTable = result.data.data;
                    this.$message.success('查询成功');
                    this.showAddTable = true
                });
        ...
        async postCourse(cno) {
            try {
                await axios.post(`/UntaughtCourses?tno=${this.userData.uno}&cno=${cno}`);
                console.log('数据提交成功！');
            } catch (error) {
        ...
        batchPost() {
            ...
            if (selectedCourses.length === 0) {
                this.$message.warning('请先选择要添加的课程');
                return;
            }
            axios.post(`/UntaughtCourses?tno=${this.userData.uno}&cnos=${selectedCourses.join(',')}`)
                .then(result => {
                    console.log(result)
                    this.refresh()
        ...
        refresh() { //刷新
            ...
            axios.get("/UntaughtCourses").then((result) => {
                this.untaughtCourses = result.data.data;
    ...
    computed: {
        totalRows() {
            return this.untaughtCourses.length; // 总数据条数
        },
        paginatedCoursesData() {
            // 根据当前页数和每页显示的数量，计算截取的数据段
            const startIndex = (this.currentPage - 1) * this.pageSize;
            const endIndex = startIndex + this.pageSize;
            return this.untaughtCourses.slice(startIndex, endIndex);
        },
    },
    mounted() {
        this.userData = this.$route.meta.userData;
        axios.get("/UntaughtCourses").then((result) => { // "https://mock.apifox.com/m1/3839966-0-default/UntaughtCourses"
            this.untaughtCourses = result.data.data
        }); //发送异步请求
    },
}
