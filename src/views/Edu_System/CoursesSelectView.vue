                    ...
                    <el-main>
                        ...
                        <el-form :inline="true" :model="searchForm2" class="demo-form-inline">
                            <el-form-item label="课程号">
                                <el-input v-model="searchForm2.cno" length="4" show-word-limit
                                    placeholder="按课程号选课"></el-input>
                            </el-form-item>
                            <el-form-item>
                                <el-button type="primary" @click="selectCourseBycno">查询</el-button>
                            </el-form-item>
                        </el-form>

                        <!-- 弹窗表格 -->
                        <el-dialog title="课程查询结果" :visible.sync="showAddTable">
                            ...
                            <br>
                            <el-button type="primary" @click="postCourse">添加课程</el-button>
                        </el-dialog>
                        ...
                        <el-table :data="paginatedCoursesSelected" border ref="coursesTable">
                            ...
                            <el-table-column align="center" prop="status" label="操作">
                                <template slot-scope="scope">
                                    <el-button slot="reference" @click="confirmDelete(scope.row.cno)">删除</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                        <br>
                        ...
                        <el-pagination background layout="sizes, prev, pager, next, jumper, total"
                            ...
                            :current-page="currentPage" :total="totalRows" :page-size="pageSize" :page-sizes="[1, 2, 3]">
...
export default {
    ...
    methods: {
        ...
        async selectCourseBycno() {
            if (this.searchForm2.cno) {
                axios.get(`/SearchCourses?sno=${this.userData.uno}&cno=${this.searchForm2.cno}`).then(result => {
                    this.courseTable = result.data.data;
                    this.acno = this.courseTable.cno;
        ...
        async postCourse() {
            try {
                await axios.post("/CoursesSelect", {
                    "sno": this.userData.uno,
                    "cno": this.searchForm2.cno,
                });
                console.log('数据提交成功！');
            } catch (error) {
        ...
        async searchCourseBycno() {
            if (this.searchForm.cno) {
                axios.get(`/CoursesSelect?sno=${this.userData.uno}&cno=${this.searchForm.cno}`).then(result => {
                    this.courseTable = result.data.data;
                    this.$message.success('查询成功');
                    this.showTable = true
                });
        ...
        async searchCourseBycname() {
            if (this.searchForm.cname) {
                axios.get(`/CoursesSelect?sno=${this.userData.uno}&cname=${this.searchForm.cname}`).then(result => {
                    this.courseTable = result.data.data;
                    this.$message.success('查询成功');
                    this.showTable = true
                });
        ...
        async deleteCourse(cno) {
            ...
            const uno = userData.uno;
            axios.delete(`/CoursesSelect?sno=${uno}&cno=${cno}`).then(result => {
        ...
        batchDelete() {
            const selectedCourses = this.$refs.coursesTable.selection.map(course => course.cno);
            if (selectedCourses.length === 0) {
                this.$message.warning('请先选择要删除的课程');
                return;
            }
            axios.delete(`/CoursesSelect?sno=${this.userData.uno}&cnos=${selectedCourses.join(',')}`)
                .then(result => {
                    console.log(result)
                    this.refresh()
        ...
        deselect() {
            this.$refs.coursesTable.clearSelection();
        },
    ...
    computed: {
        totalRows() {
            return this.coursesSelected.length; // 总数据条数
        },
        paginatedCoursesSelected() {
            // 根据当前页数和每页显示的数量，计算截取的数据段
            const startIndex = (this.currentPage - 1) * this.pageSize;
            const endIndex = startIndex + this.pageSize;
            return this.coursesSelected.slice(startIndex, endIndex);
        },
    },
    mounted() {
